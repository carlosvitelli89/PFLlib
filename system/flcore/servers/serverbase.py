...
class Server(object):
    ...
    def set_clients(self, clientObj):
        ...
        self.size = np.array([client.train_samples for client in self.all_clients])    
    ...
    def select_clients(self):
        ...
        elif self.select_client == 'entropy':

            select = self.clients
            num_select = round(len(select) * self.join_ratio)
            num_select_size = int(0.4 * num_select)
        ...
        #Seleciona com peso dos tamanho do dataset
            ...
            num_select_size_client = int(0.2 * len(select))
        ...
        #Seleciona com peso da entropia
            selected_clients = list(np.random.choice(clients_select_size, num_select_size_client,
                                                  p=weigth_entropy, replace=False))
        
  
        elif self.select_client == 'loss':            
            client_data_sizes = [(client.train_samples, client) for client in self.all_clients] #lista com samples dataset e id cliente
            client_data_sizes.sort(key=lambda x: x[0]) #sort baseado nas samples

            num_clients_top_40_percent = int(len(client_data_sizes) * 0.40) #40% de todos os clientes 
            clients_in_top_40_percent_by_size = [item[1] for item in client_data_sizes[-num_clients_top_40_percent:]] #seleciona o cliente [1] da lista com samples dataset e id cliente, do maior pro menor dataset

            losses_top_40_percent = np.array([self.client_losses.get(client.id, 1.0) for client in clients_in_top_40_percent_by_size]) #array com as losses dos clientes dentro dos 40%

            weights_top_40_percent = 1 / (losses_top_40_percent + 1e-8) #atribui os pesos para cada cliente    
            weights_top_40_percent /= weights_top_40_percent.sum() #normaliza os pesos


            num_to_select_from_top_40_percent = int(len(client_data_sizes) * 0.20) #20% dos 40% do dataset
            

            selected_clients = list(np.random.choice(
                    clients_in_top_40_percent_by_size, 
                    size=num_to_select_from_top_40_percent,
                    replace=False, 
                    p=weights_top_40_percent)) # Usar os pesos calculados para esses 40%
            


        if self.actual_round==98:
            print(self.actual_round)
            current_num_join_clients = 100
            selected_clients = list(np.random.choice(self.clients, current_num_join_clients, replace=False))
            self.calculate_entropy()
            self.all_clients = selected_clients #logica rafael primeiro round com todos os clientes
    ...
    def receive_models(self):
        assert (len(self.selected_clients) > 0) # Esta asserção agora deve passar!

        # --- MODIFICAÇÃO NECESSÁRIA AQUI (no receive_models) ---
        # Calcula quantos clientes ativos teremos com base na lista REALMENTE selecionada
        num_active_clients = int(len(self.selected_clients) * (1 - self.client_drop_rate))

        # Garantir que não tentamos amostrar mais do que temos disponível, ou um número negativo
        num_active_clients = max(0, min(num_active_clients, len(self.selected_clients)))

        active_clients = random.sample(self.selected_clients, num_active_clients)
        self.uploaded_ids = []
        self.uploaded_weights = []
        self.uploaded_models = []
        tot_samples = 0
